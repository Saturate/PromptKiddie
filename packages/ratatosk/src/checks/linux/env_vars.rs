...
fn check_ld_preload(findings: &mut Vec<Finding>) {
    ...
    if let Ok(content) = fs::read_to_string("/etc/ld.so.preload") {
        let libs: Vec<&str> = content.lines()
        ...
        if let Ok(meta) = fs::metadata("/etc/ld.so.preload") {
            ...
            if mode & 0o002 != 0 || (meta.uid() == my_uid && my_uid != 0) {
                findings.push(Finding {
                    ...
                    path: Some("/etc/ld.so.preload".into()),
                    exploit_hint: Some("add a malicious .so to intercept all dynamically linked programs".into()),
                });
            }
        }
    }
                ...
                if let Ok(meta) = fs::metadata(dir) {
                    ...
                    if mode & 0o002 != 0 || (meta.uid() == my_uid && my_uid != 0) {
                        findings.push(Finding {
                            ...
                            exploit_hint: Some("place malicious shared library to hijack dynamic linking".into()),
                        });
                    }
                }
            }
    ...
    if let Ok(path) = std::env::var("PATH") {
                ...
                if let Ok(meta) = fs::metadata(dir) {
                    ...
                    if mode & 0o002 != 0 || (meta.uid() == my_uid && my_uid != 0) {
                        findings.push(Finding {
                            ...
                            path: Some(dir.to_string()),
                            exploit_hint: Some("if root runs a command without full path, your binary executes".into()),
                        });
                    }
                }
            }
        ...
        if path.contains("::") || path.starts_with(':') || path.ends_with(':') {
            findings.push(Finding {
                ...
                path: None,
                exploit_hint: Some("place trojan in a directory where root might cd and run commands".into()),
            });
        }
    }
}
