...
const GTFOBINS_SUID: &[&str] = &[
    ...
    "perl", "ruby", "node", "php",
    "vim", "vi", "nano", "ed", "less", "more",
    "find", "nmap", "awk", "gawk", "mawk",
    "env", "strace", "ltrace", "gdb",
    "cp", "mv", "dd", "tar", "zip", "unzip",
    "wget", "curl", "nc", "ncat", "socat",
    "doas", "pkexec", "start-stop-daemon",
    "tee", "xargs", "time", "timeout",
    "docker", "aa-exec", "ab", "agetty",
    "ar", "arj", "base32", "base64",
    "busybox", "byebug", "capsh",
    "chmod", "chown", "chroot",
    "crontab", "csvtool", "cut",
    "date", "diff", "dmsetup",
    "emacs", "expect", "file",
    "flock", "fmt", "fold",
    "gimp", "grep", "head",
    "hexdump", "highlight", "iconv",
    "install", "ionice", "ip",
    "jjs", "join", "jq", "jrunscript",
    "ksu", "ld.so", "logsave",
    "look", "lua", "make",
    "msfconsole", "nice", "nl",
    "nohup", "nsenter", "openssl",
    "paste", "pdb", "perlbug",
    "pg", "pico", "rsync",
    "run-parts", "rview", "rvim",
    "screen", "script", "sed",
    "setarch", "shuf", "sort",
    "split", "ssh-keygen", "ssh-keyscan",
    "strings", "su", "sysctl",
    "systemctl", "tail", "taskset",
    "tclsh", "telnet", "tftp",
    "troff", "ul", "unexpand",
    "uniq", "unshare", "valgrind",
    "watch", "xdotool", "xxd",
];
        ...
        for dir in &search_dirs {
            for entry in WalkDir::new(dir).max_depth(5).into_iter().filter_map(|e| e.ok()) {
                ...
                let (severity, hint) = if is_suid && owner_uid == 0 && is_gtfobins {
                    (Severity::Critical, Some(format!("GTFOBins SUID: {filename} owned by root")))
                ...
                let flag = if is_suid && is_sgid { "SUID+SGID" }
                    else if is_suid { "SUID" }
                    else { "SGID" };
