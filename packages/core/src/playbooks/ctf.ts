...
const phases: PlaybookPhaseTemplate[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { key: "post.root_flag", title: "Capture root flag", type: "mechanical", command: "pk exec -- find / -name root.txt 2>/dev/null | head -5 && cat /root/root.txt 2>/dev/null || dir C:\\Users\\Administrator\\Desktop\\root.txt 2>nul", dependsOn: ["post.privesc", "post.lateral"], priority: 35 },
      { key: "post.freestyle", title: "Additional post-exploitation", type: "judgment", description: "Anything the playbook missed: persistence mechanisms, data exfiltration paths, additional flags, pivoting to other hosts. Note successful techniques for playbook improvement.", dependsOn: ["post.root_flag", "post.internal_net", "post.crack", "post.lateral"], priority: 90, optional: true },
      { key: "post.end", title: "Post-Exploitation Complete", type: "mechanical", nodeType: "sequence", dependsOn: ["post.root_flag", "post.internal_net", "post.crack", "post.lateral", "post.freestyle"], priority: 99 },
    ],
  },
  {
    phase: "report",
