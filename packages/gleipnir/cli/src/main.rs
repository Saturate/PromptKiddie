...
async fn main() {
    ...
    match cli.command {
        Commands::Sessions => {
            match c.get("/api/sessions").await {
                Ok(v) => print_json(&v),
                Err(e) => eprintln!("error: {e}"),
            }
        }
        Commands::Exec { session, command, timeout } => {
            let body = serde_json::json!({ "command": command, "timeout": timeout });
            match c.post(&format!("/api/sessions/{session}/exec"), &body).await {
                Ok(v) => {
        ...
        Commands::Listeners => {
            match c.get("/api/listeners").await {
                Ok(v) => print_json(&v),
                Err(e) => eprintln!("error: {e}"),
            }
        }
        Commands::Info => {
            match c.get("/api/info").await {
                Ok(v) => print_json(&v),
                Err(e) => eprintln!("error: {e}"),
            }
        }
        Commands::Kill { session } => {
            match c.delete(&format!("/api/sessions/{session}")).await {
                Ok(v) => print_json(&v),
                Err(e) => eprintln!("error: {e}"),
            }
        }
        Commands::Close { id } => {
            match c.delete(&format!("/api/listeners/{id}")).await {
                Ok(v) => print_json(&v),
                Err(e) => eprintln!("error: {e}"),
            }
        }
                            ...
                            if let Some(arr) = sessions.as_array() {
                                ...
                                if !connected.is_empty() {
                                    ...
                                    for s in connected {
                                        ...
                                        let host = s.get("hostname").and_then(|v| v.as_str()).unwrap_or("?");
